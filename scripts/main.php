...
if (is_logged()) {
    ...
    // get sites
    if (is_admin()) {
        $sites = \e::db_getrows("SELECT s.* FROM <<tp>>site as s");
    } else {
        $sites = \e::db_getrows("SELECT s.* FROM <<tp>>site_user as su INNER JOIN <<tp>>site as s ON su.site_id=s.id WHERE su.user_id={$_SESSION['user_info']['id']}");
    }
    $cnt = count($sites);
    for ($i = 0; $i < $cnt; $i++) {
