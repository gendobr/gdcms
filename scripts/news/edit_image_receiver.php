...
$news_id = \e::cast('integer', \e::request('news_id', 0));
...
foreach ($_FILES as $imagefile) {
    ...
    $bigFileName = "news-{$news_id}-{$lang}-{$now}-" . \core\fileutils::encode_file_name($imagefile['name']);
    if (move_uploaded_file($imagefile['tmp_name'], "{$dir}/{$bigFileName}")) {

        $smallFileName = "news-{$news_id}-{$lang}-{$now}-small-" . \core\fileutils::encode_file_name($imagefile['name']);
        $img->resize(
            "{$dir}/{$bigFileName}", "{$dir}/{$smallFileName}", 
