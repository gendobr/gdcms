...
if (isset($input_vars['save_changes']) && strlen($input_vars['save_changes']) > 0) {
    ...
    if ($debug) {
        prn('NEWS ABSTRACT:', $input_vars['page_abstract']);
    }
    ...
    if ($debug) {
        prn('NEWS CONTENT:', $input_vars['page_content']);
    }
    ...
    if (strlen($_REQUEST['expiration_date_posted']['month']) > 0 && strlen($_REQUEST['expiration_date_posted']['day']) > 0 && strlen($_REQUEST['expiration_date_posted']['year']) > 0
    ) {
        ...
        $this_news_info['expiration_date'] = '';
        ...
        if (isset($_REQUEST['notify']))
            if (is_array($_REQUEST['notify'])) {
                ...
                function allowed_chars($str) {
                    return wordwrap(preg_replace('/[^ 1234567890\-_абвгдежзийклмнопрстуфхцчшщюэяьъё.\?\!АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧЩАЮЯЬЪQWERTYUIOPASDFGHJKL;:\'ZXCVBNM,\/qwertyuiopasdfghjklzxcvbnmіїєІЇЄґҐ@%]/', ' ', strip_tags($str)), 60, '<br>');
                }
                ...
                foreach ($this_site_info['managers'] as $mng) {
                    ...
                    if (isset($_REQUEST['notify'][$mng['id']])) {
                        ...
                        $mng_body = "Dear {$mng['full_name']} <br/>\n<br/>\n<br/>\n" .
                                ...
                                " ";
                        if (IsHTML != '1')
                            $mng_body = wordwrap(strip_tags(preg_replace('/<br\/?>/i', "\n", $mng_body)), 80, "\n");
