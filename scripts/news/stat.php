...
$re->add_field($field = 'news.news_views'
        , $alias = 'news_views'
        , $type = 'integer'
        , $label = text('news_views')
        , $_group_operation = false);

$re->add_field($field = 'DATEDIFF(now(), news.last_change_date) '
        , $alias = 'news_days_old'
        , $type = 'integer'
        , $label = text('news_days_old')
        , $_group_operation = false);


//
$re->add_field($field = " IFNULL(  LENGTH( news.content )=0, 0 )"
        , $alias = 'only_abstract'
        , $type = "enum:1={$text['positive_answer']}&0={$text['negative_answer']}"
        , $label = text('only_abstract')
        , $_group_operation = false);
...
$input_vars['page_title'] =
$input_vars['page_header'] =  $this_site_info['title'] . ' - ' . text('News_views_stat');
...
for ($i = 0; $i < $cnt; $i++) {
    //--------------------------- context menu -- begin ------------------------
    $response['rows'][$i]['context_menu'] = '';
    
    //--------------------------- context menu -- end --------------------------
    $response['rows'][$i]['last_change_date'] = date('Y-m-d',strtotime($response['rows'][$i]['last_change_date']));
