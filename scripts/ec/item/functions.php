...
function get_ec_item_info($ec_item_id,$lang,$site_id=0,$use_cache=true,$ec_item_code='') {
    ...
    $query="SELECT ec_item.*,
          ...
          FROM {$table_prefix}ec_item AS ec_item
           ...
           WHERE ec_item.ec_item_lang='{$lang}'
             AND ( ec_item_id=$ec_item_id OR ec_item_code='".  DbStr($ec_item_code)."')
            ";
    // prn($query);
    ...
    $tor['additional_categories']=db_getrows($query);
    ...
    $tor['ec_item_variant']=db_getrows($query);
    ...
    for($i=0;$i<$cnt;$i++) {
        ...
        if(   isset($tor['ec_item_variant'][$i+1])
           && $tor['ec_item_variant'][$i+1]['ec_item_variant_indent']>$tor['ec_item_variant'][$i]['ec_item_variant_indent']) {
    ...
    $query="UPDATE {$table_prefix}ec_item
            SET cached_info='".DbStr(serialize($tor))."',
                cache_datetime=now()
...
function ec_item_parse_price_correction($str) {
    ...
    if(preg_match($regexp,$str,$regs)) {
        ...
        if(trim($regs[3])=='%') {
            $units='%';
            $regs[3]='';
            if($regs[1]=='-') $regs[2]=1-0.01*$regs[2]; else $regs[2]=1+0.01*$regs[2];
            $regs[1]='*';
            //prn($regs);
        }
...
function ec_item_delete($ec_item_id,$ec_item_lang) {
    ...
    if(!$this_ec_item_info) return false;

    $site_id=$this_ec_item_info['site_id'];
    $this_site_info = get_site_info($site_id);
    if(get_level($site_id)==0) return false;
...
function ec_item_clone($ec_item_id,$ec_item_lang) {
    ...
    $ec_item=db_getonerow($query);
    if(!$ec_item){
        return;
    }

    $query="SELECT * FROM  {$GLOBALS['table_prefix']}site WHERE id={$ec_item['site_id']}";
    $site_info=db_getonerow($query);
    $site_dir=sites_root.'/'.$site_info['dir'];
    ...
    unset(  $ec_item['ec_item_id'],
            //$ec_item['ec_item_img'],
            $ec_item['ec_item_uid'],
            $ec_item['ec_item_purchases'],
            $ec_item['ec_item_keywords'],
            $ec_item['ec_item_views'],
            $ec_item['ec_item_in_cart']);
    $fld=Array();
    $val=Array();
    ...
    $newid=db_getonerow("SELECT LAST_INSERT_ID() as newid");
    $newid=$newid['newid'];

    // ensure that ec_item_code is unique
    $query="UPDATE {$GLOBALS['table_prefix']}ec_item "
            . "SET ec_item_code='".  DbStr($ec_item['ec_item_code'].'-'.$ec_item['ec_item_lang'].'-'.$newid)."' "
            . "WHERE ec_item_lang='".  DbStr($ec_item['ec_item_lang'])."' AND ec_item_id={$newid}";
    db_execute($query);
    ...
    $ec_item_category=db_getrows($query);
    ...
    $ec_category_item_field_value=db_getrows($query);
    ...
    $ec_item_variant=db_getrows($query);
...
function ec_item_add_translation($ec_item_id,$ec_item_lang){
    ...
    if($new_language=='') return false;

    // ------------------------- main item record - begin ----------------------
    $query="SELECT * FROM  {$GLOBALS['table_prefix']}ec_item WHERE ec_item_id={$ec_item_id} AND ec_item_lang='{$ec_item_lang}'";
    $ec_item=db_getonerow($query);
    $ec_item['ec_item_lang']=$new_language;
    unset(  $ec_item['ec_item_purchases'],
            $ec_item['ec_item_keywords'],
            $ec_item['ec_item_views'],
            $ec_item['ec_item_in_cart'],
            $ec_item['cached_info']);
...
function ec_item_adjust($_info,$this_site_info) {
    ...
    if(!isset($currency_titles)) {
        $tmp=db_getrows("SELECT * FROM {$table_prefix}ec_currency");
    ...
    $_info['url_details']=str_replace(
            Array('{ec_item_lang}','{ec_item_id}','{ec_item_code}'),
            Array($_info['ec_item_lang'],$_info['ec_item_id'],$_info['ec_item_code']),
            url_ec_item_details_pattern);
    ...
    if(!isset($_info['additional_categories'])) $_info['additional_categories']=Array();
