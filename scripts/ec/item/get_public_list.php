...
if(isset($input_vars['ec_item_date_year']) && strlen($input_vars['ec_item_date_year'] )>0) $restriction.="  AND YEAR(ec_item.ec_item_last_change_date)=".((int)$input_vars['ec_item_date_year']);
if(isset($input_vars['ec_item_date_month'])&& strlen($input_vars['ec_item_date_month'])>0) $restriction.="  AND MONTH(ec_item.ec_item_last_change_date)=".((int)$input_vars['ec_item_date_month']);
if(isset($input_vars['ec_item_date_day'])  && strlen($input_vars['ec_item_date_day']  )>0) $restriction.="  AND DAYOFMONTH(ec_item.ec_item_last_change_date)=".((int)$input_vars['ec_item_date_day']);
...
if(isset($input_vars['ec_category_id'])) {
    if($input_vars['ec_category_id']>0) {
        $categories=explode(',',$input_vars['ec_category_id']);
...
if(    isset($input_vars['ec_item_tags'])
        && strlen($input_vars['ec_item_tags'])>0 ) {
...
$rows=24;
if(isset($input_vars['rows']) && $rows>0 && $rows<1000) $rows=(int)$input_vars['rows'];
...
if(isset($input_vars['orderby'])) {
    ...
    if(in_array($fld,$ordering_variants)) $orderby="ORDER BY $fld $od";
}
else {
    // default ordering
