...
$cats = \e::db_getrows("SELECT DISTINCT rozdil FROM <<tp>>photogalery WHERE site = '$site_id'  ORDER BY `rozdil` ASC");
$tm = Array();
foreach ($cats as $c_t) {
    $tm[$c_t['rozdil']] = $c_t['rozdil'];
}
...
if (strlen($show_category) > 0) {
    $show_category_condition = " AND LOCATE('" . \e::db_escape($show_category) . "',rozdil)=1";
} else {
    $show_category_condition = '';
}
# ----------------------- category filter - end --------------------------------
# ----------------------- get number of images - begin -------------------------
$result1 = \e::db_getonerow("SELECT count(*) as n FROM <<tp>>photogalery WHERE site = <<integer site_id>> $show_category_condition",['site_id'=>$site]);
$num = $result1['n'];
...
for ($i = 0; $i < $num; $i = $i + 10) {
    if ($i == $start)
        ...
        $pages.="<a href={$pages_url_prefix}{$i}>" . (1 + $i / 10) . "</a>\n";
}
# ----------------------- get number of images - end ---------------------------



$rows = \e::db_execute("SELECT SQL_CALC_FOUND_ROWS * FROM {$table_prefix}photogalery   WHERE site = '$site' $show_category_condition ORDER BY `id` DESC LIMIT $start, 10");
...
//--------------------------- ���� �������� --------------------------
$input_vars['page_title'] = $input_vars['page_header'] = $this_site_info['title'] . ' - ' . $text['Gallery_manage'];
