...
if ($message_set_visible > 0) {
    $query = "UPDATE <<tp>>forum_msg SET is_visible=1 WHERE id={$message_set_visible} AND site_id={$site_id} LIMIT 1";
    //prn($query);
    \e::db_execute($query);
}
//-------------------- make visible - end --------------------------------------
...
if ($message_set_invisible > 0) {
    $query = "UPDATE <<tp>>forum_msg SET is_visible=0 WHERE id={$message_set_invisible} AND site_id={$site_id} LIMIT 1";
    //prn($query);
    \e::db_execute($query);
}
//-------------------- make ibvisible - end --------------------------------------
