...
if(strlen($input_vars['interface_lang'])>0) $input_vars['lang']=$input_vars['interface_lang'];
...
# ------------------ get user info - begin -------------------------------------
  ...
  $this_user_info=db_getonerow("SELECT * FROM {$table_prefix}forum_user WHERE id={$user_id}");
  if(!$this_user_info) die('User not found');
# ------------------ get user info - end ---------------------------------------
...
$query=   
   "SELECT {$table_prefix}forum_thread.* 
...
<table>
  ...
  <tr>
    <td><b>{$txt['forum_thread_subject']}</b></td>
    <td><INPUT type='text' NAME='subject'  value='про ' SIZE='25' style='width:350px;'></td>
  </tr>
