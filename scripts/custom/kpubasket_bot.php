...
$parsed_data=Array('title'=>'unknown','komand�'=>Array(),'tur'=>Array());
$cnt=count($data);
for($i=0;$i<$cnt;$i++)
{
    ...
    # process command
      switch($tmp[0])
      {
          ...
          case 'komand�':
              $tmp[1]=explode('=',$tmp[1]);
              $parsed_data['komand�'][$tmp[1][0]]=Array(
                       'title'=>$tmp[1][1]
                      ,'stats'=>Array(
                          '�'=>0
                         ,'�'=>0
          ...
          case 'igra':
              ...
              if(isset($tmp[1][1])>0 && !isset($parsed_data['komand�'][$tmp[1][1]])) echo "<b style='color:red;'>Invalid team1 in line #$i : {$data[$i]}</b><br>";
              if(isset($tmp[1][2])>0 && !isset($parsed_data['komand�'][$tmp[1][2]])) echo "<b style='color:red;'>Invalid team2 in line #$i : {$data[$i]}</b><br>";
                  ...
                  foreach($tur['igra'] as $i=>$igra)
                  {
                      ...
                      if(strcmp($igra['data_igry'],$dt)==0)
                      {
                          ...
                          $show.="<div>
                              ...
                              {$parsed_data['komand�'][$cmd1]['title']}
                              -
                              {$parsed_data['komand�'][$cmd2]['title']}
                             </div>";
                      }
                  }
              }
          ...
          case 'last_match':
              ...
              foreach($parsed_data['tur'] as $t=>$tur)
              {
                  foreach($tur['igra'] as $i=>$igra)
                  {
                      ...
                      if(strcmp($igra['data_igry'],$dt)==0)
                      {
                         if(strlen($igra['urlstats'])>0) $igra['urlstats']="(<a href=\"{$igra['urlstats']}\">�����</a>)";

                         $show.="<div>
                              {$tur['title']};
                              ".date('d.m.Y',strtotime($dt)).";
                              {$parsed_data['komand�'][$igra['komanda1']]['title']}
                              -
                              {$parsed_data['komand�'][$igra['komanda2']]['title']};
              ...
              foreach($parsed_data['tur'] as $t=>$tur)
              {
                  ...
                  foreach($tur['igra'] as $i=>$igra)
                  {
                      ...
                      $show.="<tr>
                              ...
                              <td>".(strlen($igra['komanda1'])>0?$parsed_data['komand�'][$igra['komanda1']]['title']:'')."</td>
                              <td><nobr>{$igra['schet']} {$igra['urlstats']}</nobr></td>
                              <td>".(strlen($igra['komanda2'])>0?$parsed_data['komand�'][$igra['komanda2']]['title']:'')."</td>
                              </tr>";
                      //
                  }
              }
          ...
          case 'table':
              ...
              foreach($parsed_data['tur'] as $t=>$tur)
              {
                  foreach($tur['igra'] as $i=>$igra)
                  {
                      ...
                      $parsed_data['komand�'][$igra['komanda1']]['stats']['�']++;
                      $parsed_data['komand�'][$igra['komanda2']]['stats']['�']++;

                      $parsed_data['komand�'][$igra['komanda1']]['stats']['+']+=$schet[0];
                      $parsed_data['komand�'][$igra['komanda1']]['stats']['-']+=$schet[1];

                      $parsed_data['komand�'][$igra['komanda2']]['stats']['-']+=$schet[0];
                      $parsed_data['komand�'][$igra['komanda2']]['stats']['+']+=$schet[1];

                      if($schet[0]>$schet[1])
                      {
                         $parsed_data['komand�'][$igra['komanda1']]['stats']['�']++;
                         $parsed_data['komand�'][$igra['komanda1']]['stats']['�']+=2;

                         $parsed_data['komand�'][$igra['komanda2']]['stats']['�']++;
                         $parsed_data['komand�'][$igra['komanda2']]['stats']['�']+=1;
                      }
                      else
                      {
                         $parsed_data['komand�'][$igra['komanda1']]['stats']['�']++;
                         $parsed_data['komand�'][$igra['komanda1']]['stats']['�']+=1;

                         $parsed_data['komand�'][$igra['komanda2']]['stats']['�']++;
                         $parsed_data['komand�'][$igra['komanda2']]['stats']['�']+=2;
                      }
              ...
              function cmp($a, $b) 
              {
                ...
                return ($a['stats']['�'] < $b['stats']['�']) ? 1 : -1;
              }
              $turtable=$parsed_data['komand�'];
              uasort($turtable, "cmp");
