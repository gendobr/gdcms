...
if (!$this_page_info) {
    $input_vars['page_title'] = $input_vars['page_header'] = $input_vars['page_content'] = $text['Page_not_found'];
    return 0;
}
...
if (isset($input_vars['save_changes']))
    if (strlen($input_vars['save_changes']) > 0) {
            ...
            if (count($_FILES) > 0) {
                if (isset($_FILES['page_upload']))
                    if ($_FILES['page_upload']['size'] > 0) {
                        ...
                        if (preg_match('/<\\/body/', $this_page_info['content'])) {
        ...
        if (!isset($input_vars['page_is_under_construction']))
            $input_vars['page_is_under_construction'] = 0;
                    ...
                    foreach ($this_site_info['managers'] as $mng) {
                        if (isset($_REQUEST['notify'][$mng['id']])) {
                            ...
                            $mng_body = "Dear {$mng['full_name']} <br/>\n<br/>\n<br/>\n" .
                                    ...
                                    " ";
                            if (IsHTML != '1')
                                $mng_body = wordwrap(strip_tags(preg_replace("/<br\\/?>/", "\n", $mng_body)), 80, "\n");
...
$input_vars['page_content'].="
  ...
  <div class=big>
      <div>
        <a href=\"javascript:void(0)\" onclick=\"display_gallery_links('index.php?action=photo/json&lang={$this_page_info['lang']}&site_id={$site_id}',this)\" style=\"display:inline-block;\">" . text('Gallery') . "</a>
...
# ------------------------ insert links into editor - begin --------------------
    ...
    <script type=\"text/javascript\" charset=\"" . site_charset . "\" src=\"".\e::config('APPLICATION_ADMIN_URL')."/scripts/lib/markitup/jquery.markitup.js\"></script>
    <script type=\"text/javascript\" charset=\"" . site_charset . "\" src=\"".\e::config('APPLICATION_ADMIN_URL')."/scripts/lib/markitup/sets/html/set.js\"></script>
    <script type=\"text/javascript\" charset=\"" . site_charset . "\" src=\"".\e::config('APPLICATION_ADMIN_URL')."/scripts/lib/markitup.js\"></script>
    <script type=\"text/javascript\" charset=\"" . site_charset . "\" src=\"".\e::config('APPLICATION_ADMIN_URL')."/scripts/lib/choose_links.js\"></script>
