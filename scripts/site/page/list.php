...
//-------------------- delete page - begin -------------------------------------
$delete_page_id = \e::cast('integer', \e::request('delete_page_id', 0));
if ($delete_page_id > 0) {
    $delete_page_info = get_page_info($delete_page_id, \e::db_escape(\e::request('delete_page_lang')));
    if ($delete_page_info) {
...
foreach ($tmp as $tm)
    $list_of_categories[] = $tm['category_id'] . '=' . rawurlencode(str_repeat(' + ', $tm['deep']) . get_langstring($tm['category_title']));
unset($tmp, $tm);
