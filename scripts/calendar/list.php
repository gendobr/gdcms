...
if (isset($input_vars['event_delete']) && isset($input_vars['event'])) {
    ...
    \e::db_execute("DELETE FROM <<tp>>calendar WHERE id in($ids)");
    \e::db_execute("DELETE FROM <<tp>>calendar_category WHERE event_id in($ids)");
    \e::db_execute("DELETE FROM <<tp>>calendar_date WHERE calendar_id in($ids)");
    \e::db_execute("DELETE FROM <<tp>>calendar_cache WHERE uid between {$site_id}000000 AND {$site_id}999990");
    \e::db_execute("DELETE FROM <<tp>>calendar_days_cache WHERE calendar_id in($ids)");
}
// --------------------- delete - end ------------------------------------------
#---------------------------- hide - begin -------------------------------------
if (isset($input_vars['event_hide']) && isset($input_vars['event'])) {
    $ids = join(',', array_map('toint', $input_vars['event']));
    \e::db_execute("UPDATE <<tp>>calendar SET vis=0 WHERE id in($ids) AND site_id={$site_id}");
    \e::db_execute("DELETE FROM <<tp>>calendar_cache WHERE uid between {$site_id}000000 AND {$site_id}999990");
}
#---------------------------- hide - end ---------------------------------------
#---------------------------- show - begin -------------------------------------
if (isset($input_vars['event_show']) && isset($input_vars['event'])) {
    $ids = join(',', array_map('toint', $input_vars['event']));
    \e::db_execute("UPDATE <<tp>>calendar SET vis=1 WHERE id in($ids) AND site_id={$site_id}");
    \e::db_execute("DELETE FROM <<tp>>calendar_cache WHERE uid between {$site_id}000000 AND {$site_id}999990");
}
#---------------------------- show - end ---------------------------------------
...
if(count($whereDate)>0){
    $where[]=" id in(select calendar_id FROM <<tp>>calendar_date WHERE ".join(" AND ",$whereDate).") ";
}
...
$num =\e::db_getonerow("SELECT count(*) as n FROM <<tp>>calendar WHERE site_id=$site_id $where");
$num = $num['n'];
...
$rows = \e::db_getrows("SELECT * FROM <<tp>>calendar WHERE site_id='$site_id' $where ORDER BY `id` ASC LIMIT $start, 100");
...
if ($rows) {
    ...
    if(count($ids)>0){
        $query="SELECT * FROM <<tp>>calendar_date WHERE calendar_id IN (".join(',',$ids).") ORDER BY calendar_id";
