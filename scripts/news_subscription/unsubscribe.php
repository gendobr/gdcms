...
if(isset($input_vars['code'])){
    ...
    if($subscriber_info){
        ...
        $query="DELETE FROM <<tp>>news_subscriber WHERE news_subscriber_code='$code'";
        \e::db_execute($query);
        // show message
...
if(isset($input_vars['news_subscriber_email']) && strlen($input_vars['news_subscriber_email'])>0 && is_valid_email($input_vars['news_subscriber_email'])){
    ...
    $subscriber_info=\e::db_getonerow("select * from <<tp>>news_subscriber WHERE news_subscriber_email='".\e::db_escape($input_vars['news_subscriber_email'])."'");
    if($subscriber_info){
