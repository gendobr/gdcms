...
if (isset($input_vars['new_notifier_email'])) {
    ...
    if (strlen($new_listener_event) == 0)
        $message .= '<div style="color:red;">' . text('ERROR_new_listener_event_is_empty') . '</div>';
    if (strlen($new_site_id) == 0)
        $message .= '<div style="color:red;">' . text('ERROR_new_site_id_is_empty') . '</div>';
    if (strlen($new_listener_sendto) == 0)
        $message .= '<div style="color:red;">' . text('ERROR_new_sendto_is_empty') . '</div>';
    if (!is_valid_email($new_listener_sendto))
        $message .= '<div style="color:red;">' . text('invalid_email_address') . '</div>';
...
if (isset($input_vars['new_notifier_telephone'])) {
    ...
    if (strlen($new_site_id) == 0)
        $message .= '<div style="color:red;">' . text('ERROR_new_site_id_is_empty') . '</div>';
    if (strlen($input_vars['new_notifier_telephone']) == 0)
        $message .= '<div style="color:red;">' . text('ERROR_new_telephone_is_empty') . '</div>';
...
foreach ($tmp as $ev)
    $listener_event_options[$ev] = $ev . '=' . rawurlencode(text('Notifier_event_' . $ev));
...
$listener_event_options = Array('' => '');
foreach ($tmp as $ev)
    $listener_event_options[$ev] = text('Notifier_event_' . $ev);
...
$input_vars['page_title'] = $input_vars['page_header'] = text('List_of_notifiers');
?>
