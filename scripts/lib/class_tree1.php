...
// additional conditions to select
    ...
    function move_to($category_id = 0) {
        ...
        if ($cid > 0) {
            ...
            $destination_info = \e::db_getonerow($query);
        } else {
        ...
        # category cannot be moved into itself
        if ($this->info[$this->name_start] <= $destination_info[$this->name_start] && $destination_info[$this->name_finish] <= $this->info[$this->name_finish]
        ) {
            return false;
        }
    ...
    function move_down($category_id = 0) {
        ...
        # get category info
        $category_info = \e::db_getonerow("SELECT * FROM {$this->name_table} WHERE {$this->name_id}={$cid} " . $this->sql_where());
        if (!$category_info)
            return false;
        ...
        $query = "SELECT *
                 ...
                 LIMIT 0,1";
        $parent_info = \e::db_getonerow($query);
        if (!$parent_info)
            return false;
    ...
    function move_up($category_id) {
        ...
        $query = "SELECT ch.*
                 ...
                 WHERE {$parent_info[$this->name_start]}<ch.{$this->name_start}
                     ...
                     " . $this->sql_where('ch') . "
                 ORDER BY ch.{$this->name_start}";
        $children = array_reverse(\e::db_getrows($query));
    ...
    function delete_branch($category_id = 0) {
        ...
        $query = "SELECT {$this->name_id} as id
              ...
              WHERE {$_info[$this->name_start]}<={$this->name_start}
                ...
                " . $this->sql_where();
        $deleted_ids = \e::db_getrows($query);
    ...
    function get_parents($condition = '') {
        ...
        $this->parents = \e::db_getrows($query);
    ...
    function get_children($condition = '') {
        ...
        $this->children = \e::db_getrows($query);
    ...
    function load_node($_id = 0, $condition = '') {
        ...
        if ($cid > 0) {
            ...
            //prn($query);
            $this->info = \e::db_getonerow($query);
        } else {
            $this->info = false;
        }
