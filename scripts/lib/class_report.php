...
// ------------------ add field to list -- begin -------------------------------------------------------------------
    function add_field($_field, $_alias, $_ttype, $_label, $_group_operation = false) {
        ...
        if (isset($this->field[$_alias]))
            echo "<font color=red><b>field ($_field,$_alias,$_ttype,$_label, $_group_operation) already exists</b></font><br>";
        ...
        switch ($_type) {
            case 'string':
                ...
                if (get_magic_quotes_gpc())
                    $input_vars[$varname] = stripslashes($input_vars[$varname]);
                ...
                if (strlen($input_vars[$varname]) > 0) {
                    if ($_group_operation)
                        $this->having[] = " ( {$_field} LIKE '%{$input_vars[$varname]}%' ) ";
                    else
                        $this->where[] = " ( {$_field} LIKE '%" . \e::db_escape($input_vars[$varname]) . "%' ) ";
                }
                break;
            ...
            case 'enum':
                ...
                if (strlen($input_vars[$varname]) > 0) {
                    if ($_group_operation)
                        $this->having[] = " ( {$_field}='" . \e::db_escape($input_vars[$varname]) . "' ) ";
                    else
                        $this->where[] = " ( {$_field}='" . \e::db_escape($input_vars[$varname]) . "' ) ";
                }
                break;
            ...
            case 'id':
                ...
                if (preg_match('/^[0-9]+$/', $input_vars[$varname])) {
                    ...
                    if ($_group_operation)
                        $this->having[] = " ( {$_field}={$input_vars[$varname]} ) ";
                    else
                        $this->where[] = " ( {$_field}={$input_vars[$varname]} ) ";
                }
                break;
            ...
            case 'unix_timestamp':
                ...
                if (strlen($input_vars[$varname]) > 0) {
                    if (!(($timestamp = strtotime($input_vars[$varname])) === -1)) {
                        ...
                        if ($_group_operation)
                            $this->having[] = " ( {$_field}>={$timestamp} ) ";
                        else
                            $this->where[] = " ( {$_field}>={$timestamp} ) ";
                    }
                }
...
// ---------------------- create query to run -- begin ---------------------------------------------------------
    function create_query() {
        ...
        // ----------- ordering -- begin ----------------------------------------
        if (isset($GLOBALS['_REQUEST']['orderby']))
            $this->orderby = $GLOBALS['_REQUEST']['orderby'];
        else
            $this->orderby = '';
...
// --------------------- show response -- begin --------------------------------
    function show() {
        ...
        if ($ans < 0)
            $ans = 0;
        if ($start > 0)
            $to_show['backward'] = "{$_SERVER['PHP_SELF']}?{$newquery}&start={$ans}";
        ...
        foreach ($this->field as $fld) {
            if ($fld['type'] == 'enum')
                $positions[] = Array($posa, $fld['alias']);
            ++$posa;
        }
...
// --------------------- show response -- end ----------------------------
    ...
    function checkStr($tostr) {
        if (isset($tostr))
            return trim(htmlspecialchars($tostr, ENT_QUOTES, 'cp1251'));
        else
            return '';
    }
