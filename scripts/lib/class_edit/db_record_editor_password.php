...
class db_record_editor_field_password extends db_record_editor_field_string{
  ...
  function check_custom($password)
  {
    ...
    if($password==$_REQUEST["{$this->form_element_name}_again"]) return '';

    return sprintf('<b><font color=red>ERROR: %s has invalid format</font></b> ',$this->label);
  }

  function sql_value(){ return "'".md5($this->value)."'"; }
