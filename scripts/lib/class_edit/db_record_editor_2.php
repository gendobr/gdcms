...
// --------------------- set primary key -- begin ------------------------------
    function set_primary_key($_keyname, $_keyvalue = '') {
        ...
        if (!$this->field[$_keyname]->check($_keyvalue))
            return false;
...
// ------------------ get basic info from database -- end ----------------------
    ...
    function save() {
        ...
        if ($this->record_is_found) {# ----------------- update record -- begin --------------------------------
            $fff = Array();
            //prn($this);
            foreach ($this->field as $fld) {
                if ($fld->primary_key)
...
# ----------------- draw hidden elements - end ---------------------------------
  ...
  ";
        ...
        foreach ($this->field as $el)
            $tor.=$el->draw_default_template();
  ...
  //----------------------- database interface -- end --------------------------
    ...
    function form_is_posted() {
        if (!isset($_REQUEST["{$this->form_name_prefix}is_submitted"]))
            return false;
        return $_REQUEST["{$this->form_name_prefix}is_submitted"] == 'yes';
    }

    function value_of($field_name) {
