...
return [
    'ALTER TABLE `<<tp>>category` 
      ADD COLUMN `category_icon` TEXT NULL AFTER `date_lang_update`,
      ADD COLUMN `category_meta` TEXT NULL AFTER `category_icon`',
    
    'ALTER TABLE `<<tp>>category` ADD COLUMN `category_title_short` VARCHAR(1024) NULL AFTER `category_meta`',
    
    'ALTER TABLE `<<tp>>category` ADD COLUMN `category_description_short` TEXT NULL AFTER `category_title_short`',
    
    'ALTER TABLE `<<tp>>news` ADD COLUMN `news_views` INT DEFAULT 0  NOT NULL',
    "CREATE TABLE `<<tp>>photo_category` (
