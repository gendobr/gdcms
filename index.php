...
//------------------------- interface to posted data -- begin ------------------
  // uploaded files
     if(version_compare(phpversion(), '4.1.0', '<')){ if (version_compare(phpversion(), '3', 'eq')) $_FILES     = & $HTTP_POST_VARS;  else $_FILES     = & $HTTP_POST_FILES;}
  // REQUEST
    $input_vars = $_REQUEST;
    if(get_magic_quotes_gpc()){ function remove_magic_quotes($iv){ $tor=Array(); $cnt=array_keys($iv); foreach($cnt as $key) { if(is_array($iv[$key])) $tor[$key]=remove_magic_quotes($iv[$key]); else $tor[$key]=stripslashes($iv[$key]);} return $tor; } $input_vars=remove_magic_quotes($input_vars); }
...
if($db)
{
    if(defined('db_encoding')) db_execute('set names '.db_encoding);
