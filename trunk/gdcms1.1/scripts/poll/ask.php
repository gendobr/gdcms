...
if(isset($input_vars['poll']) && is_array($input_vars['poll']) ) {
    if(isset($input_vars[$md5_headers]) && isset($_COOKIE['poll_last_answer']) && ( (time()-$_COOKIE['poll_last_answer']) > $poll_timeout)) {
        ...
        $query="SELECT count(id) as n,
                     ( IF(client_ip   ='$ip',1,0)
                     + IF(client_sign ='".DbStr($input_vars[$md5_headers])."',1,0)
        ...
        if(count($answer_ids)>0) {
            db_execute("UPDATE {$table_prefix}golos_vidpovidi
                        SET golosiv=golosiv+1
                        WHERE id IN(".join(',',$answer_ids).")");

            // ----------------- save details - begin --------------------------
            // existing answers
            db_execute("insert into {$table_prefix}golos_vidpovidi_details(poll_id, answer_id, session_id, answer_date,site_id,client_ip,client_sign,client_sign2,client_is_valid,poll_uid)
                        SELECT v.pynannja_id, v.id, '{$session_id}', now(), $site_id,'$ip','".DbStr($input_vars[$md5_headers])."','{$md5_headers}',$client_is_valid,'{$poll_uid}'
            ...
            if(count($missing_answers)>0) {
                db_execute("insert into {$table_prefix}golos_vidpovidi_details(
                              poll_id, answer_id, session_id,
                              answer_date,site_id,client_ip,
                              client_sign,client_sign2,client_is_valid,poll_uid)
	                   SELECT p.id, 0,'{$session_id}', now(), $site_id,'$ip','".DbStr($input_vars[$md5_headers])."','{$md5_headers}',$client_is_valid,'{$poll_uid}'
