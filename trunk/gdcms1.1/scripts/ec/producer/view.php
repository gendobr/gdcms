...
# --------------------------- get producer info - begin ------------------------
  ...
  if(!$this_producer_info) die('Producer not found');
...
if(!isset($_SESSION['code']) || strlen($_SESSION['code'])==0)  $_SESSION['code']=get_code();
...
$comment_body='';
$messages='';
if(isset($input_vars['comment_sender'])) $comment_sender=trim(strip_tags($input_vars['comment_sender']));
if(isset($input_vars['comment_body']))   $comment_body=trim(strip_tags($input_vars['comment_body']));
if(isset($input_vars['comment_code']))
{
   if(strlen($comment_body)==0) $messages.="<div style='color:red;'>".text('EC_item_comment_error_empty_comment')."</div>";
   if(trim($input_vars['comment_code'])!=$_SESSION['code'])  $messages.="<div style='color:red;'>".text('EC_item_comment_error_invalid_code')."</div>";
   if(strlen($comment_sender)==0) $comment_sender='Anonymous';
...
# ------------------------ draw using SMARTY template - begin ------------------
  ...
  $vyvid=
  process_template( $template_ec_producer_view
                    ,Array(
                           'text'=>$txt,
                           'site'=>$this_site_info,
