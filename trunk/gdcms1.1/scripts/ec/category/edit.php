...
#------------------------ get category info - begin ----------------------------
 ...
 # prn('$this_category->info',$this_category->info);
   if(!$this_category->info) unset($input_vars['ec_category_id']);

 }

 if(!isset($input_vars['ec_category_id'])) redirect_to('index.php?action=ec/category/list');
...
#                 'is_deleted'
#                ,'is_deleted'
#                ,'integer(1='.rawurlencode('��').'&0='.rawurlencode('���').')'
#                ,'�������');
...
if($this_category->info['start']>0)
{
  ...
  $tmp=Array();
  foreach($list_of_categories as $ct) $tmp[]=$ct['ec_category_id'].'='.rawurlencode(str_repeat(' + ',$ct['deep']).get_langstring($ct['ec_category_title']));
  //prn($tmp);
...
$rep->field['is_visible']=new db_record_editor_field_integer(
                 'is_visible'
                ,'is_visible'
                ,"integer(0=".rawurlencode('���')."&1=".rawurlencode('��').")"
 ...
 # these handlers are located in ec/item/functions.php
   ...
   for($i=0;$i<$cnt;$i++)
   {
       if(!preg_match('/^onnullamount_/',$tmp['user'][$i])) {unset($tmp['user'][$i]);continue;}
...
#  ---------------------------- draw - begin -----------------------------------
  ...
  $input_vars['page_header']  = text('ec_category_edit');
