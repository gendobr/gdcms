...
function run_function($params, &$smarty) {
    ...
    extract($params);
    $p = eregi_replace('[^a-z0-9_]', '_', $name);
...
/*
  Цель этого фрагмента - сравнение двух URL
  чтобы подсвечивать пункты меню

  Т.е. если URL пункта меню совпадает с URL текущей страницы,
  пункт меню подсвечивается(иногда. в зависимости от шаблона)

  Аргументы -
  1) URL текущей страницы
  2) URL пункта меню

  URL пункта меню может быть
  - относительным ( начинается с ./ или с ../ [0-9a-z~_-] )
  - от корня сервера (начинается с /)
  - абсолютным (начинается с https?://)
