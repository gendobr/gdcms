...
// ---------------------- user-defined functions -- begin ----------------------
  function check_form_values() {return true;}
  function process_db_values() {return true;}
  function create_primary_key(){return $this->db->GenID("{$this->table}_seq");}
        ...
        switch($_type)
        {
           case 'string':
               if(isset($posted_data))
               {
                  ...
                  if(!isset($tor['options']['required'])) $tor['options']['required']='no';
           ...
           case 'enum':
               ...
               if(!isset($posted_data)) $posted_data='';
           ...
           case 'integer':
               ...
               {
                  $posted_data='';
                  if(isset($tor['options']['default'])) $posted_data=$this->checkInt($tor['options']['default']);
               }
           ...
           case 'float':
               ...
               {
                  if(isset($tor['options']['default'])) $posted_data=$this->checkFloat($tor['options']['default']);
               }
               $tor['value']=$tor['form_element_value']=$posted_data;
           break;
...
// --------------------- create GET query -- begin -----------------------------
   function create_get_query($to_exclude='')
   {
       ...
       foreach($PARAM as $k0=>$v0)
          if(!eregi($to_exclude, $k0)) $newquery[]="{$k0}=".rawurlencode($v0);
          ...
          switch($this->field[$key]['type'])
          {
             case 'datetime':
               if(strlen($result[$key])>0)
               {
                 ...
                 $this->field[$key]['form_element_value']=$this->checkDatetime($result[$key]);
               }
             break;
...
// ------------------ get basic info from database -- end ----------------------
  ...
  function set_table($_tname){$this->table=$_tname;               }
  ...
  function checkStr($ffff)  {if(isset($ffff)) return htmlspecialchars($ffff, ENT_QUOTES,'cp1251'); else return '';}
  function DbStr($ffff)
  {
     if(isset($ffff))
     {
       //$tor=str_replace("'","''",$ffff);
       $tor=str_replace("\\","\\\\",$ffff);
       $tor=str_replace("'","\\'",$tor);
       //prn("{$ffff} => {$tor} ");
       return $tor;
  ...
  {
     if (!(($timestamp = strtotime($ffff)) === -1)) return date($this->datetime_format,$timestamp);
     else return date($this->datetime_format);
  }
  ...
  function prn($tst){echo '<hr><pre>'; print_r($tst); echo '</pre><hr>';}
  ...
  function save()
  {
     ...
     // -------------------- additional checks -- begin ------------------------
        if(! ($this->check_form_values() && $this->all_is_ok)) return false;
     ...
     if(!$this->record_is_found)
     {
        // ----------------- create record -- begin ----------------------------
           ...
           foreach($this->field as $fld)
           {
               ...
               ///else $val=$fld['value'];
               if(isset($fld['primary_key']) && $fld['primary_key']) $primary_key_alias=$fld['alias'];
               else
               switch($fld['type'])
               {
                   case 'string':
            ...
            if(count($fff)>0)
            {
              ...
              else
              {
                 ...
                 if($retcode)
                 {
                   ...
                   $this->field[$primary_key_alias]['value']=
                   $this->field[$primary_key_alias]['form_element_value']=
     ...
     else
     {
        // ----------------- update record -- begin ----------------------------
           ...
           foreach($this->field as $fld)
           {
               if(!isset($fld['primary_key'])) $fld['primary_key']=false;
               if($fld['primary_key'])
               {
                 if($fld['type']=='string') $this->where_str=" WHERE {$fld['field']}='{$this->id}' ";
                 else $this->where_str=" WHERE {$fld['field']}={$this->id} ";
               }
               else
               {
                 switch($fld['type'])
                 {
                     ...
                     case 'datetime':
                     case 'enum':
                         $fff[]="{$fld['field']}='".$this->DbStr($fld['value'])."'";
                     break;
...
function draw_options($value,$options)
{
  ...
  foreach($options as $key=>$val)
  {
    if(is_array($val))
    {
        if($this->eq($val[0],$value)) $selected=' selected '; else $selected='';
        $to_return.="<option value=\"".checkStr(trim($val[0]))."\" $selected>{$val[1]}</option>\n";
    }
    else
    {
       if($this->eq($key,$value)) $selected=' selected '; else $selected='';
